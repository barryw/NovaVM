...
@Suite struct DocumentTests {
    ...
    @Test func imageSlotsAreCappedAtTwelve() {
        let doc = NovaDocument(width: 1, height: 1)

        for _ in 1..<NovaDocument.maximumImageCount {
            #expect(doc.addImage())
        }

        #expect(doc.images.count == NovaDocument.maximumImageCount)
        #expect(!doc.addImage())
    }
    ...
    @Test func backdropScaleIsClamped() {
        ...
        #expect(doc.backdropScale == NovaDocument.minimumBackdropScale)

        doc.scaleBackdrop(by: 1_000_000, around: .zero)
        #expect(doc.backdropScale == NovaDocument.maximumBackdropScale)
    }

    @Test func backdropScrollScalingUsesFineSteps() {
        let wheelFactor = NovaDocument.backdropScrollScaleFactor(
            deltaY: 3,
            hasPreciseScrollingDeltas: false
        )
        let trackpadFactor = NovaDocument.backdropScrollScaleFactor(
            deltaY: 10,
        ...
        #expect(NovaDocument.backdropScrollScaleFactor(deltaY: -3, hasPreciseScrollingDeltas: false) < 1)
    }

    @Test func initialState() {
        let doc = NovaDocument(width: 320, height: 200)
