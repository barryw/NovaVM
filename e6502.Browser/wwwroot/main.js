...
const ASSET_VERSION = "2026-06-12-novachess-tt";
...
const quickMountBtn = document.getElementById("quick-mount-btn");
...
const SHOWCASE_DISKS = [
    { id: "novachess", label: "NovaChess", path: "./showcase/novachess.ndi", runtime: "basic" },
    { id: "novaz", label: "NovaZ launcher", path: "./showcase/novaz.ndi", runtime: "basic" },
    { id: "novaz-z6-spec", label: "NovaZ Z6 spec", path: "./showcase/novaz-z6-spec.ndi", runtime: "basic" },
    { id: "novaz-z6-v7", label: "NovaZ Z6/V7 spec", path: "./showcase/novaz-z6-spec-v7.ndi", runtime: "basic" },
    { id: "novaz-z6-colours", label: "NovaZ Z6 colours", path: "./showcase/novaz-z6-colours.ndi", runtime: "basic" },
    { id: "novaz-z6-main", label: "NovaZ Z6 main returns", path: "./showcase/novaz-z6-main-returns.ndi", runtime: "basic" },
    { id: "novaz-z6-nosegment", label: "NovaZ Z6 nosegment", path: "./showcase/novaz-z6-nosegment.ndi", runtime: "basic" },
...
const workerUrl = new URL(useRustCore ? "./nova-rust-worker.js" : "./nova-worker.js", import.meta.url);
...
function initializeControls() {
    ...
    quickMountBtn?.addEventListener("click", quickMountShowcaseDisk);

    if (!useRustCore) {
        setHostStatus(".NET browser core selected");
        if (quickMountBtn) quickMountBtn.disabled = true;
    }
}

function populateShowcaseDisks() {
    if (!showcaseSelect) return;
    showcaseSelect.textContent = "";
    for (const disk of SHOWCASE_DISKS) {
        const option = document.createElement("option");
        option.value = disk.id;
        option.textContent = disk.label;
        showcaseSelect.appendChild(option);
    }
}

async function quickMountShowcaseDisk() {
    if (!useRustCore) {
        setHostStatus("quick mount requires the Rust core");
        return;
    }

    const disk = SHOWCASE_DISKS.find(item => item.id === showcaseSelect?.value) || SHOWCASE_DISKS[0];
    if (!disk) return;

    try {
        if (quickMountBtn) quickMountBtn.disabled = true;
        setHostStatus(`mounting ${disk.label}`);
        const bytes = await fetchArrayBuffer(disk.path);
        mountedMedia.fd0 = { name: diskName(disk.path), size: bytes.byteLength, showcase: true };
        selectedRuntime = disk.runtime || selectedRuntime;
        if (runtimeSelect) runtimeSelect.value = selectedRuntime;
        if (autobootCheck) autobootCheck.checked = true;
        setQueryParam("runtime", selectedRuntime);
        setQueryParam("noAuto", "0");
        sendControl({ command: "mount", drive: "fd0", name: diskName(disk.path), bytes }, [bytes]);
        sendControl({ command: "reboot", runtime: selectedRuntime, skipAutoboot: false });
        setHostStatus(`booting ${disk.label}`);
        screen.focus();
    } catch (error) {
        showError(`quick mount failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
        if (quickMountBtn) quickMountBtn.disabled = false;
    }
}
...
function diskName(path) {
    return path.split("/").pop() || "disk.ndi";
}
