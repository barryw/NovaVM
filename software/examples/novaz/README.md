...
```sh
make -C software/examples/novaz
```

Build an `.ndi` without a story, useful for checking the runtime handoff:

```sh
make -C software/examples/novaz ndi
```

The default image is `software/examples/novaz/dist/fd0.ndi`, matching the NovaHost
auto-mount name for an inserted FD0 floppy.

Build an `.ndi` with a local story file:

```sh
make -C software/examples/novaz ndi STORY=/path/to/zork1.z3
```

Build one of the initial story projects:

```sh
cp /path/to/your/zork1.z3 software/examples/novaz/projects/zork-i/STORY.BIN
...
```sh
make -C software/examples/novaz smoke STORY=/path/to/zork1.z3
```

Run the scripted Zork I state/scroll regressions against a generated image:

```sh
make -C software/examples/novaz test-zork-state STORY=/path/to/zork1.z3
make -C software/examples/novaz test-zork-house STORY=/path/to/zork1.z3
make -C software/examples/novaz test-zork-save STORY=/path/to/zork1.z3
```

Run a project-specific regression, such as Deadline's time-status smoke:

```sh
make -C software/examples/novaz test-project PROJECT=deadline STORY=/path/to/deadline.z3
```
...
```sh
make -C software/examples/novaz test-infocom-z3-longplays
make -C software/examples/novaz test-infocom-v45-longplays
make -C software/examples/novaz test-infocom-longplays
```
...
```sh
make -C software/examples/novaz test-z3-spec
```
...
```sh
make -C software/examples/novaz test-z4-styles
```
...
```sh
make -C software/examples/novaz test-z5-spec
```
...
```sh
make -C software/examples/novaz test-z4-spec
```
...
```sh
make -C software/examples/novaz test-z6-suite
```
...
```sh
make -C software/examples/novaz test-spec-suites
```
...
```sh
cp /path/to/your/zork0.z6 software/examples/novaz/projects/zork-zero/STORY.BIN
make -C software/examples/novaz test-project PROJECT=zork-zero
```
...
```sh
make -C software/examples/novaz test-project-save-restore PROJECT=zork-zero \
...
Run `make -C software/examples/novaz test-z6-spec` beside that when changing generic V6
