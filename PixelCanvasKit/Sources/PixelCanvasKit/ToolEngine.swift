...
/// Applies pointer input (mouse down/drag/up) to a ``PixelDocument`` according
/// to its `currentTool`: pencil, eraser, shapes, flood fill, eyedropper, and a
/// floating-selection move/copy/paste model. Platform-agnostic — callers feed
/// it integer canvas coordinates; it owns no view state.
@Observable
@MainActor
public final class ToolEngine {
    public let document: PixelDocument

    private var dragStart: (x: Int, y: Int)?
    private var dragShift = false
    private var previewPixels: PixelSnapshot?
    ...
    private struct FloatingSelection {
        let width: Int
        let height: Int
        let pixels: PixelSnapshot
        let basePixels: PixelSnapshot
        let originalPixels: PixelSnapshot
    ...
    public init(document: PixelDocument) {
        self.document = document
    }

    public func mouseDown(x: Int, y: Int, shift: Bool) {
    ...
    public func mouseDragged(x: Int, y: Int, shift: Bool) {
    ...
    public func eraseMouseDown(x: Int, y: Int) {
        dragStart = (x, y)
        dragShift = false
        previewPixels = nil
        document.pushUndo()
    ...
    public func eraseMouseDragged(x: Int, y: Int) {
    ...
    public func pasteClipboard(at target: (x: Int, y: Int)? = nil) {
        discardFloatingSelectionIfImageChanged()
        ...
        document.pushUndo()
        let snapshot = PixelSnapshot(pixels: clip.pixels, paintedPixels: clip.paintedPixels)
    ...
    public func nudgeSelection(dx: Int, dy: Int) {
        discardFloatingSelectionIfImageChanged()
        guard dx != 0 || dy != 0,
    ...
    public func cancelSelectionAction() {
        discardFloatingSelectionIfImageChanged()
    ...
    private func capturePixels(in sel: (x: Int, y: Int, w: Int, h: Int)) -> PixelSnapshot {
        ...
        return PixelSnapshot(pixels: pixels, paintedPixels: paintedPixels)
    }

    private func stampPixels(_ snapshot: PixelSnapshot, atX px: Int, y py: Int, width: Int, height: Int) {
        for y in 0..<height {
            for x in 0..<width {
                let index = y * width + x
    ...
    public func drawLine(x0: Int, y0: Int, x1: Int, y1: Int, color: UInt8, paints: Bool = true) {
        var x0 = x0, y0 = y0
        let dx = abs(x1 - x0)
        let dy = -abs(y1 - y0)
        let sx = x0 < x1 ? 1 : -1
    ...
    public func drawRect(x0: Int, y0: Int, x1: Int, y1: Int, color: UInt8, filled: Bool) {
    ...
    public func drawCircle(cx: Int, cy: Int, r: Int, color: UInt8, filled: Bool) {
    ...
    public func floodFill(x: Int, y: Int, newColor: UInt8) {
