...
/// Zoom / pan math for displaying a ``PixelDocument`` in a view of a given
/// pixel size. Pure geometry: every method reads and writes the document's
/// `zoom` and `panOffset`; it draws nothing itself.
@MainActor
public enum CanvasViewport {
    public static let minimumZoom: CGFloat = 1
    public static let maximumZoom: CGFloat = 256
    public static let zoomStops: [CGFloat] = [1, 2, 4, 8, 16, 32, 64, 128, 256]
    ...
    public static func fitZoom(document: PixelDocument, size: CGSize) -> CGFloat {
    ...
    public static func fitToView(document: PixelDocument, size: CGSize) {
        document.zoom = fitZoom(document: document, size: size)
        centerCanvas(document: document, size: size)
    }

    public static func applyZoom(document: PixelDocument, size: CGSize, zoom: CGFloat, anchor: CGPoint) {
    ...
    public static func applyCenteredZoom(document: PixelDocument, size: CGSize, zoom: CGFloat) {
        let newZoom = clampedZoom(zoom)
    ...
    public static func centerCanvas(document: PixelDocument, size: CGSize) {
        document.panOffset = centeredPanOffset(document: document, size: size)
    }

    public static func clampPanOffset(document: PixelDocument, size: CGSize) {
        centerCanvas(document: document, size: size)
    }

    public static func pan(document: PixelDocument, by delta: CGPoint) {
    ...
    public static func nextZoomStop(after zoom: CGFloat) -> CGFloat {
        let current = clampedZoom(zoom)
        return zoomStops.first { $0 > current * 1.01 } ?? maximumZoom
    }

    public static func previousZoomStop(before zoom: CGFloat) -> CGFloat {
        let current = clampedZoom(zoom)
        return zoomStops.last { $0 < current / 1.01 } ?? minimumZoom
    }

    private static func centeredPanOffset(document: PixelDocument, size: CGSize) -> CGPoint {
        guard size.width > 0, size.height > 0 else {
