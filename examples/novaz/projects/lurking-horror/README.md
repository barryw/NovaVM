...
```text
examples/novaz/projects/lurking-horror/STORY.BIN   # or pass STORY=...
...
```sh
make -C examples/novaz jukebox-lurking
```

Build the game itself (story + sounds):

```sh
make -C examples/novaz ndi PROJECT=lurking-horror STORY="/path/to/Lurking.z3"
```
...
```sh
make -C examples/novaz test-project PROJECT=lurking-horror STORY="/path/to/Lurking.z3"
```
