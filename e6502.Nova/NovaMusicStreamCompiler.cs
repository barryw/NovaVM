...
internal static class NovaMusicStreamCompiler
{
    ...
    private const ushort NmsHeaderSize = 128;
    private const int NmsTextBytes = 32;
    ...
    public sealed record Metadata(string Title, string Author, string Copyright);
    ...
    public static Result Compile(string midiPath)
    {
        ...
        Metadata metadata = ExtractMetadata(midi);
        ...
        writer.Write(0u);
        WritePaddedAscii(writer, metadata.Title, NmsTextBytes);
        WritePaddedAscii(writer, metadata.Author, NmsTextBytes);
        WritePaddedAscii(writer, metadata.Copyright, NmsTextBytes);
        if (ms.Length != NmsHeaderSize)
            throw new InvalidOperationException("internal NMS header size mismatch.");
        writer.Write(events.ToArray());
        return new Result(ms.ToArray(), events.Count / NmsEventRecordSize,
            totalFrames, metadata);
    }

    private static Metadata ExtractMetadata(MidiFile midi)
    {
        string title = "";
        string author = "";
        string copyright = "";
        ...
        foreach (TrackChunk track in midi.GetTrackChunks())
        {
            foreach (MidiEvent ev in track.Events)
            {
                if (string.IsNullOrWhiteSpace(title) &&
                    ev is SequenceTrackNameEvent trackName)
                    title = trackName.Text;
                if (string.IsNullOrWhiteSpace(copyright) &&
                    ev is CopyrightNoticeEvent copyrightEvent)
                    copyright = copyrightEvent.Text;
                if (string.IsNullOrWhiteSpace(title) && ev is TextEvent text)
                    title = text.Text;
        ...
        return new Metadata(CleanMetadataText(title),
            CleanMetadataText(author),
            CleanMetadataText(copyright));
    }
    ...
    private static void WritePaddedAscii(BinaryWriter writer, string text, int len)
    {
