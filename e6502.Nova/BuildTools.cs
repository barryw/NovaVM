...
internal static class NovaBuildTools
{
    ...
    public static void GenerateNdkReference(string runtimeDir, string texPath, string jsonPath, string? pascalDir = null)
    {
        ...
        if (pascalDir is not null)
            WriteNdkPascalBindings(libraries, runtimeDir, pascalDir);
    ...
    private static void WriteNdkPascalBindings(
        IEnumerable<Dictionary<string, object?>> libraries,
        string runtimeDirectory,
        string outputDirectory)
    {
        List<Dictionary<string, object?>> libraryList = libraries.ToList();
        Dictionary<string, List<Dictionary<string, object?>>> stemGroups = libraryList
            .GroupBy(library => NdkPascalFileStem(Convert.ToString(library["name"], CultureInfo.InvariantCulture)!))
            .ToDictionary(group => group.Key, group => group.ToList(), StringComparer.Ordinal);
        string[] duplicateStems = stemGroups.Where(group => group.Value.Count > 1).Select(group => group.Key).ToArray();
        if (duplicateStems.Length > 0)
            throw new InvalidOperationException($"Pascal unit filename collision: {string.Join(", ", duplicateStems)}");

        Dictionary<string, string> routineOwners = libraryList
            .SelectMany(library => ((List<Dictionary<string, object?>>)library["entries"]!)
                .Where(entry => Value(entry, "kind").Equals("routine", StringComparison.OrdinalIgnoreCase)
                                && NdkPascalRoutineSignature(entry) is not null)
                .Select(entry => new
                {
                    Symbol = Value(entry, "symbol").ToUpperInvariant(),
                    Stem = NdkPascalFileStem(Convert.ToString(library["name"], CultureInfo.InvariantCulture)!),
                }))
            .Where(item => item.Symbol.Length > 0)
            .GroupBy(item => item.Symbol)
            .ToDictionary(group => group.Key, group => group.First().Stem, StringComparer.Ordinal);

        var markerSymbols = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (Dictionary<string, object?> library in libraryList)
        {
            string name = Convert.ToString(library["name"], CultureInfo.InvariantCulture)!.ToUpperInvariant();
            string stem = NdkPascalFileStem(name);
            string guard = "NPI_" + stem;
                         ...
                         .OrderBy(group => group.Key.Symbol, StringComparer.Ordinal))
            {
                ...
                lines.Add($"; {symbol}");
                lines.Add($".ifndef {marker}");
                lines.Add(declaration);
                lines.Add(".endif");
            }
            lines.Add(".endif");
            WriteText(Path.Combine(outputDirectory, stem + ".NPI"), string.Join("\n", lines) + "\n");
            WriteNdkPascalContract(library, stem, outputDirectory);
            WriteNdkPascalFacades(library, stem, routineOwners, runtimeDirectory, outputDirectory);
        }
    }

    private static void WriteNdkPascalContract(
        Dictionary<string, object?> library,
        string stem,
        string outputDirectory)
    {
        string name = Convert.ToString(library["name"], CultureInfo.InvariantCulture)!;
        var entries = (List<Dictionary<string, object?>>)library["entries"]!;
        var lines = new List<string>
        {
            $"unit Nova{string.Concat(name.Split('_').Select(part => char.ToUpperInvariant(part[0]) + part[1..].ToLowerInvariant()))};",
            "",
            "interface",
            "",
            "{ Generated from the canonical NDK metadata. Do not edit. }",
        };

        Dictionary<string, Dictionary<string, object?>> constants = entries
            .Where(entry => Value(entry, "kind").Equals("const", StringComparison.OrdinalIgnoreCase)
                            && AddressToInt(entry.GetValueOrDefault("resolved")) is <= byte.MaxValue)
            .Where(entry => Value(entry, "symbol").Length > 0)
            .GroupBy(entry => Value(entry, "symbol").ToUpperInvariant())
            .ToDictionary(group => group.Key, group => group.First(), StringComparer.Ordinal);
        if (constants.Count > 0)
        {
            lines.Add("const");
            foreach ((string symbol, Dictionary<string, object?> entry) in constants.OrderBy(pair => pair.Key, StringComparer.Ordinal))
                lines.Add($"  {symbol} = ${AddressToInt(entry.GetValueOrDefault("resolved"))!.Value:X2};");
            lines.Add("");
        }

        string[] storage = entries
            .Where(entry => Value(entry, "kind") is "u8" or "zp")
            .Select(entry => Value(entry, "symbol").ToUpperInvariant())
        ...
        {
            lines.Add("var");
            foreach (string symbol in storage)
                lines.Add($"  {symbol}: Byte; external;");
            lines.Add("");
        }

        foreach (IGrouping<string, Dictionary<string, object?>> group in entries
                     .Where(entry => Value(entry, "kind").Equals("routine", StringComparison.OrdinalIgnoreCase))
                     .Where(entry => Value(entry, "symbol").Length > 0)
                     .GroupBy(entry => Value(entry, "symbol").ToUpperInvariant())
        ...
        {
            int?[] signatures = group.Select(NdkPascalRoutineSignature).Distinct().ToArray();
            if (signatures.Length != 1 || signatures[0] is not int signature)
                continue;
            string argument = (signature & 1) != 0 ? "Value: Byte" : "";
            if ((signature & 2) != 0)
                lines.Add($"function {group.Key}({argument}): Byte; external;");
            else
                lines.Add(argument.Length > 0
                    ? $"procedure {group.Key}({argument}); external;"
                    : $"procedure {group.Key}; external;");
        }
        ...
        lines.Add("");
        lines.Add("end.");
        WriteText(Path.Combine(outputDirectory, $"NOVA{stem}.PAS"), string.Join("\n", lines) + "\n");
    }

    private static void WriteNdkPascalFacades(
        Dictionary<string, object?> library,
        string stem,
        IReadOnlyDictionary<string, string> routineOwners,
        string runtimeDirectory,
        string outputDirectory)
    {
        var sources = ((IEnumerable<string>)library["sources"]!).ToArray();
        var entries = (List<Dictionary<string, object?>>)library["entries"]!;
        Dictionary<string, Dictionary<string, object?>[]> sourceEntries = entries
            .Where(entry => Value(entry, "kind").Equals("routine", StringComparison.OrdinalIgnoreCase)
                            && NdkPascalRoutineSignature(entry) is not null)
            .GroupBy(entry => Path.GetFileName(Value(entry, "source")), StringComparer.OrdinalIgnoreCase)
            .ToDictionary(group => group.Key, group => group.ToArray(), StringComparer.OrdinalIgnoreCase);
        string[] implementationSources = sources
            .Where(source => source.EndsWith(".s", StringComparison.OrdinalIgnoreCase))
            .Where(source => sourceEntries.TryGetValue(source, out Dictionary<string, object?>[]? sourceRoutines)
                             && sourceRoutines.Any(entry => routineOwners[Value(entry, "symbol").ToUpperInvariant()] == stem))
            .ToArray();
        foreach (string source in implementationSources)
        {
            string[] foreignSymbols = sourceEntries[source]
                .Where(entry => routineOwners[Value(entry, "symbol").ToUpperInvariant()] != stem)
                .Select(entry => Value(entry, "symbol"))
                .ToArray();
            if (foreignSymbols.Length > 0)
                throw new InvalidOperationException($"{source}: mixes Pascal routines owned by multiple units ({string.Join(", ", foreignSymbols)})");
        }
        var implementationDependencies = new HashSet<string>(entries
            .Where(entry => Value(entry, "kind").Equals("routine", StringComparison.OrdinalIgnoreCase)
                            && NdkPascalRoutineSignature(entry) is not null)
        ...
        foreach (string source in sources.Where(source => source.EndsWith(".s", StringComparison.OrdinalIgnoreCase)))
        {
            bool emitsSource = emittedSourceNames.Contains(source);
            string text = File.ReadAllText(Path.Combine(runtimeDirectory, source));
            foreach (Match match in Regex.Matches(text, "(?im)^\\s*\\.include\\s+\"([^\"]+\\.inc)\""))
            {
                string include = Path.GetFileName(match.Groups[1].Value);
                if (NdkPascalFileStem(Path.GetFileNameWithoutExtension(include)) == stem)
                    interfaceSources.Add(include);
                else
                {
            ...
            if (!emitsSource)
                continue;
            foreach (Match match in Regex.Matches(text, "(?im)^\\s*\\.include\\s+\"([^\"]+\\.s)\""))
            {
        ...
        string[] distinctInterfaceSources = interfaceSources.Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
        foreach (string source in distinctInterfaceSources)
        {
            string text = File.ReadAllText(Path.Combine(runtimeDirectory, source));
            foreach (Match match in Regex.Matches(text, "(?im)^\\s*\\.include\\s+\"([^\"]+\\.inc)\""))
            {
        ...
        {
            "; Generated from canonical NDK sources. Do not edit.",
            $".ifndef PINC_{stem}",
            $"PINC_{stem} = 1",
        };
        inc.AddRange(externalIncludes.Order(StringComparer.Ordinal).Select(include => $".include \"{include}\""));
        foreach (string source in distinctInterfaceSources)
        {
            inc.Add("");
            inc.Add(RewriteNdkIncludes(File.ReadAllText(Path.Combine(runtimeDirectory, source))));
        }
        inc.Add(".endif");
        WriteText(Path.Combine(outputDirectory, stem + ".INC"), string.Join("\n", inc) + "\n");

        var implementation = new List<string>
        {
            "; Generated from canonical NDK sources. Do not edit.",
            $".ifndef PIMPL_{stem}",
            $"PIMPL_{stem} = 1",
        ...
        {
            implementation.Add("");
            implementation.Add(RewriteNdkImplementationIncludes(File.ReadAllText(Path.Combine(runtimeDirectory, source))));
        }
        implementation.AddRange(implementationDependencies.Order(StringComparer.Ordinal).Select(owner => $".include \"{owner}.S\""));
        implementation.Add(".endif");
        WriteText(Path.Combine(outputDirectory, stem + ".S"), string.Join("\n", implementation) + "\n");
    }

    private static string RewriteNdkIncludes(string text)
    {
        string normalized = text.Replace("\r\n", "\n", StringComparison.Ordinal).Replace('\r', '\n');
        return Regex.Replace(normalized, "(?im)(^\\s*\\.include\\s+\")([^\"]+)(\\.(?:inc|s))(\")", match =>
            match.Groups[1].Value
            + NdkPascalFileStem(Path.GetFileNameWithoutExtension(match.Groups[2].Value))
            + match.Groups[3].Value.ToUpperInvariant()
            + match.Groups[4].Value);
    }

    private static string RewriteNdkImplementationIncludes(string text)
    {
        string withoutEarlyImplementations = Regex.Replace(
            text,
            "(?im)^\\s*\\.include\\s+\"([^\"]+\\.s)\"[^\\r\\n]*",
            match => $"; {NdkPascalFileStem(Path.GetFileNameWithoutExtension(match.Groups[1].Value))}.S is included after this unit body.");
        return RewriteNdkIncludes(withoutEarlyImplementations);
    }

    private static string NdkPascalFileStem(string name) =>
    ...
    {
        DocEntry[] allEntries = entries.ToArray();
        var errors = new List<string>();
        foreach (DocEntry entry in allEntries)
        {
        ...
        foreach (IGrouping<string, DocEntry> group in allEntries
                     .Where(entry => entry.String("symbol").Length > 0)
                     .GroupBy(entry => entry.String("symbol").ToUpperInvariant())
                     .Where(group => group.Select(entry => entry.String("symbol")).Distinct(StringComparer.Ordinal).Count() > 1))
        {
            errors.Add($"case-insensitive symbol collision: {string.Join(", ", group.Select(entry => entry.String("symbol")).Distinct(StringComparer.Ordinal))}");
        }
        if (errors.Count > 0)
