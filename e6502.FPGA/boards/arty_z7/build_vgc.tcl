# build_vgc.tcl — non-project Vivado flow: NovaVM VGC -> HDMI OUT on Arty Z7-20.
#   vivado -mode batch -source build_vgc.tcl
# Produces build/arty_z7_vgc.bit
#
# Defines: SYNTHESIS (selects the hdl-util/hdmi OSERDESE2 path), VIDEO_720X480
# (selects the VGC 858x525 / 720x480 timing profile). LATTICE_ECP5 stays
# undefined. $readmemh paths ("rom/...") resolve via the ./rom symlink.
...
read_verilog -sv [list \
    "arty_z7_vgc.sv" \
    "$rtl_dir/vgc.sv" \
    "$rtl_dir/vgc_timing.sv" \
    "$rtl_dir/vgc_text.sv" \
    "$rtl_dir/vgc_gfx.sv" \
    "$rtl_dir/vgc_sprites.sv" \
    "$rtl_dir/vgc_copper.sv" \
    "$rtl_dir/artist.sv" \
    "$rtl_dir/dpram.sv" \
    "$rtl_dir/thirdparty/sfifo.v" \
    "$hdmi_dir/hdmi.sv" \
