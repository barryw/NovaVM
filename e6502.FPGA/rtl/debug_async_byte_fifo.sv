// Small dual-clock byte FIFO for the ESP32 SPI debug transport.
//
// The SPI pins are clocked by the ESP32 master, while debug_bridge runs in the
// pixel clock domain. This FIFO keeps those domains separated so the SPI slave
// can run from SCK instead of oversampling SCK with clk_pixel.

module debug_async_byte_fifo #(
