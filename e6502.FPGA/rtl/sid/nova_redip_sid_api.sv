...
module nova_redip_sid_api (
    input  logic        clk,
    input  sid::bus_i_t bus_i,
    input  sid::cs_t    cs,
    input  logic        model_8580,
    input  logic [3:0]  voice0_vol,
    input  logic [3:0]  voice1_vol,
    input  logic [3:0]  voice2_vol,
    output sid::reg8_t  data_o,
    output sid::reg8_t  osc3_o,
    output sid::reg8_t  env3_o,
    ...
    sid::cfg_t   sid1_cfg;
    sid::cfg_t   sid2_cfg;
    sid::model_e selected_model;
    // NB! Don't put multi-bit variables in arrays, as Yosys handles that incorrectly.
    logic [1:0] sid_cs;
    logic [1:0] model;

    always_comb begin
        selected_model = model_8580 ? sid::MOS8580 : sid::MOS6581;
        sid1_cfg = { selected_model, sid::D400, 9'd250, 11'sd0 };
        sid2_cfg = { selected_model, sid::D420, 9'd250, 11'sd0 };
    ...
    assign osc3_o = sid1_osc3;
    assign env3_o = sid1_env3;
    ...
    always_ff @(posedge clk) begin
        if (bus_i.res) begin
            ...
            sid2_osc3 <= 0;
            sid2_env3 <= 0;
        end else if (voice_cycle == 8) begin
            sid1_osc3 <= wav[11-:8];
            sid1_env3 <= env;
        end else if (voice_cycle == 11) begin
            sid2_osc3 <= wav[11-:8];
            sid2_env3 <= env;
        end
    end
    ...
    sid::s22_t dca_raw;
    sid::s22_t dca_scaled;
    logic [3:0] voice_scale;
    ...
    function automatic sid::s22_t scale_dca(input sid::s22_t sample, input logic [3:0] scale);
        logic signed [26:0] product;
        begin
            product = $signed(sample) * $signed({1'b0, scale});
            scale_dca = sid::s22_t'(product >>> 4);
        end
    endfunction

    always_comb begin
        unique case (filter_cycle)
            2: voice_scale = voice0_vol;
            3: voice_scale = voice1_vol;
            4: voice_scale = voice2_vol;
            default: voice_scale = 4'hF;
        endcase
        dca_scaled = scale_dca(dca_raw, voice_scale);
    end
    ...
    sid_filter filter (
        ...
        .voice_i (filter_cycle == 5 || filter_cycle == 10 ? ext_in_1[23-:22] : dca_scaled),
        .audio_o (filter_o)
    );
