...
module usb_hid_host #(
  ...
  parameter FULL_SPEED_CLK_HZ = 48000000,
  parameter KEYBOARD_SUPPORT = 1,
  parameter MOUSE_SUPPORT = 1,
  parameter GAME_SUPPORT = 1
) (
  input wire clk,    // FULL_SPEED_CLK_HZ clock when FULL_SPEED=1, otherwise 12MHz
...
wire       full_speed;
wire       stall_event;
wire [7:0] ukp_dbg_core_status;

reg [7:0] load_data;

ukp #(
  .FULL_SPEED(FULL_SPEED),
  .FULL_SPEED_CLK_HZ(FULL_SPEED_CLK_HZ)
) ukp (
  .reset(reset),
  .clk(clk),
  .cs(cs),
  ...
  .connerr(connerr),
  .busy(busy),
  .dbg_core_status(ukp_dbg_core_status),
...
reg [2:0] endpoint_retry;      // default IN endpoint to try when the descriptor endpoint is unknown
wire [15:0] endpoint_in_token;
...
assign dbg_core_status = {ukp_dbg_core_status[7:4], endpoint_retry, ukp_dbg_core_status[0]};
assign endpoint_in_token = default_in_token(endpoint_retry);

integer i;

function [15:0] default_in_token;
  input [2:0] endpoint;
  begin
    case (endpoint)
      3'd1: default_in_token = 16'h5881;  // IN endpoint 1 (81 58)
      3'd2: default_in_token = 16'hc101;  // IN endpoint 2 (01 c1)
      3'd3: default_in_token = 16'h7181;  // IN endpoint 3 (81 71)
      3'd4: default_in_token = 16'hba01;  // IN endpoint 4 (01 ba)
      3'd5: default_in_token = 16'h0a81;  // IN endpoint 5 (81 0a)
      3'd6: default_in_token = 16'h9301;  // IN endpoint 6 (01 93)
      3'd7: default_in_token = 16'h2381;  // IN endpoint 7 (81 23)
      default: default_in_token = 16'h5881;
    endcase
  end
endfunction
...
always @(posedge clk) begin
  if (reset) begin
    endpoint_retry     <= 3'd1;
    connected_retry_r <= 1'b0;
  end else begin
    connected_retry_r <= connected;

    if (stall_event || (connected_retry_r && !connected))
      endpoint_retry <= (endpoint_retry == 3'd7) ? 3'd1 : endpoint_retry + 3'd1;
  end
end
...
always @(*) begin
  ...
  casez ({regs[4], regs[5], regs[6], vid, pid})  // INTERFACE_CLASS, INTERFACE_SUBCLASS, INTERFACE_PROTOCOL, VID, PID
    ...
    {8'h03, 8'h01, 8'hzz, 16'hzzzz, 16'hzzzz}: begin
      if (MOUSE_SUPPORT)
        typ_next = 2;  // mouse
      else if (KEYBOARD_SUPPORT && !GAME_SUPPORT)
        typ_next = 1;  // keyboard-only builds accept non-boot keyboard reports
    end
    {8'h03, 8'hzz, 8'hzz, 16'hzzzz, 16'hzzzz}: begin
      if (GAME_SUPPORT)
        typ_next = 3;  // other (incl. 8BitDo, D-Input)
      else if (KEYBOARD_SUPPORT && !MOUSE_SUPPORT)
        typ_next = 1;  // keyboard-only builds accept generic HID
    end
  ...
  // Nova only enables keyboard support. Some full-speed keyboards reach the
  // configured/connected state while descriptor capture remains empty; keep
  // polling reports instead of suppressing them behind typ_next==0.
  if (typ_next == 0 && KEYBOARD_SUPPORT && !MOUSE_SUPPORT && !GAME_SUPPORT && connected)
    typ_next = 1;
end
...
always @(*) begin
  casez ({vid, pid})
    ...
    default: begin
      in_payload[0] = endpoint_in_token[7:0];
      in_payload[1] = endpoint_in_token[15:8];
...
localparam [2:0]  FULL_SPEED_TICKS = (FULL_SPEED_CLK_HZ == 60000000) ? 3'd5 : 3'd4;
localparam [2:0]  FULL_SPEED_TICK_MAX = FULL_SPEED_TICKS - 3'd1;
localparam [15:0] FULL_SPEED_FRAME_TICKS = (FULL_SPEED_CLK_HZ == 60000000) ? 16'd60000 : 16'd48000;
localparam [7:0]  FULL_SPEED_TIMEOUT_TICKS = FULL_SPEED_TICKS * 18;  // 18 x bit time
...
`else
wire interval_frame = interval == (FULL_SPEED ? FULL_SPEED_FRAME_TICKS : 12000);
`endif

`ifdef VERILATOR
wire timeout = 0;
`else
wire timeout = delay >= (FULL_SPEED && full_speed ? FULL_SPEED_TIMEOUT_TICKS : 144);
`endif
...
assign dbg_core_status   = {stall, nak, connected, full_speed, up, um, ug, busy};
...
always @(posedge clk) begin
  if (reset || (&conct)) begin
    ...
    // Prescale the full-speed host clock down to 12MHz for low-speed devices.
    if (FULL_SPEED && !full_speed)
      prescaler <= (prescaler == FULL_SPEED_TICK_MAX) ? 0 : prescaler + 1;
    else
      prescaler <= 0;
    ...
    if (!ug && did != di && dsum != 0)
      ...
      if (FULL_SPEED && full_speed && timing == FULL_SPEED_TICK_MAX)
        timing <= 0;
    end
    ...
    if (sample) begin
      ...
      if (bitaddr == 16) begin
        ...
        stall_event <= connected && data[3:0] == 4'b1110;
      end
