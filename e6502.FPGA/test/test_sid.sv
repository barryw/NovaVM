...
module test_sid;
    ...
    initial begin
        ...
        rst = 1;
        repeat(100) @(posedge clk);
        check("SID phase held during reset", uut.sid_phase, 0);
        check("Write FIFO cleared during reset", uut.write_fifo_count, 0);
        check("Write active cleared during reset", uut.write_active, 0);
        check("Audio held silent during reset", audio_out, 0);
        rst = 0;
        repeat(50) @(posedge clk);
