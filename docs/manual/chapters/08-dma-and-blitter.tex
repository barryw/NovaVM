...
\begin{notebox}
\cmd{DMACOPY} and \cmd{DMAFILL} are asynchronous: they start the transfer and
return immediately. Poll \cmd{DMASTATUS} until it changes from busy (1) to done
...
\begin{notebox}
\cmd{BLITCOPY} and \cmd{BLITFILL} are also asynchronous. Use
\cmd{BLITSTATUS}/\cmd{BLITERR}/\cmd{BLITCOUNT} to monitor progress and final
result.
\end{notebox}
