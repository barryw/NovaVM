...
\texttt{\$A000}--\texttt{\$A01F} &  32 B  & VGC Registers                   & R/W \\
\texttt{\$A100}--\texttt{\$A13F} &  64 B  & Network Interface Controller (NIC) & R/W \\
...
\texttt{\$BA40}--\texttt{\$BA4F} &  16 B  & Timer Controller                & R/W \\
\texttt{\$BA50}--\texttt{\$BA56} &   7 B  & Music Status (6 voices)         & RO \\
\texttt{\$BA60}--\texttt{\$BA7F} &  32 B  & DMA Controller                  & R/W \\
\texttt{\$BA80}--\texttt{\$BA9F} &  32 B  & Blitter Controller              & R/W \\
\texttt{\$BC00}--\texttt{\$BFFF} & 1024 B & XRAM Windows (when mapped)      & R/W \\
\texttt{\$C000}--\texttt{\$FFFF} &  16 KB & ROM (NovaBASIC)                 & R only \\
\texttt{\$D400}--\texttt{\$D41C} &  29 B  & SID chip 1 (write-intercepted)  & W only \\
\texttt{\$D420}--\texttt{\$D43C} &  29 B  & SID chip 2 (write-intercepted)  & W only \\
\end{tabular}
\end{center}
...
\begin{center}
\begin{tabular}{ll}
\textbf{Address} & \textbf{Purpose} \\
\hline
\texttt{\$0200}--\texttt{\$0202} & Ctrl-C flag, byte, and timeout \\
\texttt{\$0203}--\texttt{\$0204} & VEC\_CC --- Ctrl-C check vector \\
\texttt{\$0205}--\texttt{\$0206} & VEC\_IN --- input vector \\
\texttt{\$0207}--\texttt{\$0208} & VEC\_OUT --- output vector \\
\texttt{\$0209}--\texttt{\$020A} & VEC\_LD --- load vector \\
\texttt{\$020B}--\texttt{\$020C} & VEC\_SV --- save vector \\
\texttt{\$020D}--\texttt{\$0216} & IRQ handler code area (set by \cmd{IRQ}) \\
\texttt{\$0217}--\texttt{\$0220} & NMI handler code area (set by \cmd{NMI}) \\
\end{tabular}
\end{center}

Hardware controller base addresses are fixed constants; use the values from
Appendix~\ref{chap:memmap} directly in your assembly code.
...
it returns the last character received, or 0 if none. BASIC's own \cmd{GET}
...
%% ============================================================
\section{The Timer Controller}
\label{sec:timer}
%% ============================================================

The timer controller at \$BA40 generates periodic IRQ interrupts, firing once
every \textit{N} video frames (at 60~Hz). It has no dedicated BASIC keyword ---
you configure it with \cmd{POKE} and handle its interrupts with \cmd{IRQ}.
...
\hline
\texttt{\$BA40} & TimerCtrl   & Bit 0 = enable \\
\texttt{\$BA41} & TimerStatus & Bit 0 = IRQ pending (reading clears) \\
\texttt{\$BA42} & TimerDivL   & Divisor low byte \\
\texttt{\$BA43} & TimerDivH   & Divisor high byte \\
\end{tabular}
\end{center}

The timer fires an IRQ every \textit{divisor} video frames. A divisor of 60
fires once per second; a divisor of 1 fires every frame. The pending flag is
cleared automatically when you read \texttt{TimerStatus}.

\subsection*{Example: one-second heartbeat}
...
\begin{notebox}
Disable the timer (\cmd{POKE \$BA40, 0}) before changing the divisor.
The \cmd{SIDPLAY} command uses the timer internally; starting SID playback
