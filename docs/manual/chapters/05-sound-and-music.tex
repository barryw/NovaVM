...
\begin{itemize}
  \item \cmd{voice} --- voice number 1--6.
...
MUSIC PRIORITY v1{[},\ldots,v6{]} & Set voice-stealing priority for sound
  effects. Lower-numbered voices are stolen first. Default: 6,~5,~4,~3,~2,~1. \\
\bottomrule
\end{tabular}
\end{center}
...
MNOTE(voice) & The MIDI note number currently sounding on \cmd{voice}
  (1--6), or 0 if that voice is silent. \\
\bottomrule
\end{tabular}
\end{center}
...
voice. When \cmd{SOUND} triggers a sound effect, the engine:
...
and noise percussion on SID~1. The music repeats until you type \cmd{MUSIC STOP} in
