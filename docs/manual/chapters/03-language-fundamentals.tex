...
% ---------------------------------------------------------------------------
\section{Shorthand Commands}

\subsection{DEC and INC}

\cmd{INC} and \cmd{DEC} increment or decrement a variable by 1 without
...
\cmd{WAIT addr, mask [, xor]} busy-waits (polls in a tight loop) until a
memory location satisfies a bit condition:

\begin{center}
\texttt{(PEEK(\textit{addr}) XOR \textit{xor}) AND \textit{mask} <> 0}
\end{center}

The \textit{xor} parameter defaults to 0 if omitted. \cmd{WAIT} is useful for
...
\begin{lstlisting}[style=basiclisting]
10 REM Wait for DMA to finish (status leaves busy=1)
20 WAIT $BA61, 1, 1
30 REM Waits until bit 0 is CLEAR (XOR flips it)
\end{lstlisting}

\begin{warningbox}
\cmd{WAIT} blocks the BASIC interpreter completely until the condition is met.
If the condition is never satisfied, the program hangs. Use \cmd{STOP} (Ctrl+C)
to break out.
\end{warningbox}

\subsection{RESET}

\cmd{RESET} performs a full hardware reset: it stops both SID chips, halts the
...
\begin{warningbox}
  ...
  into system areas.  See the memory map in Appendix~\ref{chap:memmap} for safe zones.
\end{warningbox}

\subsection{Cursor Control}

The text cursor position is set with \cmd{LOCATE x, y} where \textit{x} is
the column (0--79) and \textit{y} is the row (0--24).

Cursor \textbf{visibility} is controlled via the VGC register at \$A00A:
