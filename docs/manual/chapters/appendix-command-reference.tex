...
The sound controller provides 4 independent channels (0--3).
...
\bottomrule
\endfoot
SOUND ch,freq,dur       & Play on channel \textit{ch} (0--3); frequency 16--12000 Hz; duration in 1/60 s ticks. \\
VOLUME level            & Set master volume (0--15; only the low nibble is used). \\
ENVELOPE ch,a,d,s,r     & Set ADSR envelope for channel \textit{ch}; sustain uses low nibble (0--15 scale). \\
WAVE ch,waveform        & Set waveform for channel \textit{ch}: 0=square, 1=sawtooth, 2=triangle, 3=noise, 4=sine. \\
\end{longtable}
...
\subsection*{Extended Two-Byte Tokens (\texttt{\$FF} prefix, \texttt{XTK\_DIR} through \texttt{XTK\_XUNMAP})}

\begin{retrobox}
\ttfamily\small
DIR, DEL, XMEM, XBANK, XPOKE, XPEEK(), STASH, FETCH, XFREE, XRESET, XALLOC,
XDIR, XDEL, XMAP, XUNMAP
\end{retrobox}
