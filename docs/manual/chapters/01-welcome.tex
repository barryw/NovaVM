...
clock at 60~Hz. This keeps \cmd{VSYNC}, raster IRQ behavior, timers, and music
sequencing synchronized to one timing model.

You can override runtime behavior with environment variables:

\begin{itemize}
  \item \texttt{NOVA\_CPU\_HZ}: target CPU rate in cycles/second (default 12,000,000).
  \item \texttt{NOVA\_TURBO}: if set to \texttt{1}, \texttt{true}, \texttt{yes}, or \texttt{on}, run unthrottled.
  \item \texttt{NOVA\_TIMING\_LOG}: if set, emit periodic timing telemetry for diagnostics.
\end{itemize}
...
\begin{itemize}
  ...
  \item Play \textbf{6-voice synthesized sound} (two SID chips) with ADSR
