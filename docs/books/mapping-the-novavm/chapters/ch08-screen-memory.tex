...
NovaVM's text layer is an 80$\times$50 display. Character RAM, Color RAM,
and Text Attribute RAM are each 4000 bytes: one byte per visible cell.
Character RAM holds the character code. Color RAM holds a packed colour
attribute: the low nibble is foreground and the high nibble is background.
Text Attribute RAM holds per-cell style flags: bit~0 is flash, bit~1 is
reverse, and bit~2 is bold.
...
character, colour, and text attribute memory goes through the VDC-style VRAM port at
...
7 & Text Attribute RAM & 4000 bytes \\
\bottomrule
\end{tabular}
\end{center}
...
\section{Display Dimmer}

\memrange{A0E5}{A0E5}{Display Dimmer}{R/W}{---}

\$A0E5 holds a 4-bit global output brightness. A value of 0 forces active
video to black; 15 is full brightness. NovaHost uses this register for the
SD-resident boot logo fade before releasing the CPU into the configured ROM.
