...
\begin{enumerate}
  ...
  \item \textbf{DMA Controller} (\$BA63--\$BA75) --- a high-speed block
        transfer engine that moves data between any combination of CPU RAM,
        VGC memory spaces, and XRAM.
  \item \textbf{Blitter Controller} (\$BA83--\$BA9B) --- a 2D block transfer
...
2 & SID source: 1 $=$ the current hosted music source uses SID voices \\
3 & WTS source: 1 $=$ the current hosted music source uses WTS/MIDI voices \\
4 & Loading: 1 $=$ hosted music or soundfont assets are loading \\
5--7 & Reserved \\
\bottomrule
\end{tabular}
\end{center}
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{DMA Controller (\$BA63--\$BA75)}
\label{sec:dma}
% ═══════════════════════════════════════════════════════════════════════════════
...
\midrule
0 & CPU RAM (64\,KB) \\
1 & VGC Character RAM (4000 bytes) \\
2 & VGC Color RAM (4000 bytes) \\
3 & VGC Graphics bitmap (64\,KB) \\
4 & VGC Sprite data (32\,KB) \\
5 & XRAM (512\,KB, 24-bit addressing) \\
6 & VGC Tile data (32\,KB) \\
\bottomrule
\end{tabular}
\end{center}
...
80 REM length = 4000 bytes (80x50 screen)
90 POKE $BA6E, $A0 : POKE $BA6F, $0F : POKE $BA70, 0
...
50 REM length = 64000 bytes (320x200, one byte per pixel)
60 POKE $BA6E, 0 : POKE $BA6F, $FA : POKE $BA70, 0
70 POKE $BA71, 1      : REM fill mode
80 POKE $BA72, $05    : REM fill value (color 5)
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{Blitter Controller (\$BA83--\$BA9B)}
\label{sec:blitter}
% ═══════════════════════════════════════════════════════════════════════════════
