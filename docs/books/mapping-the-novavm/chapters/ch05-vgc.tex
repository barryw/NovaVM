...
layers.  For newly printed text, it is packed into the high nibble of the
cell's Color RAM byte.
...
register will appear in the new colour.  For newly printed text, it is packed
into the low nibble of the cell's Color RAM byte.
...
sprite $N$ for sprites 0--7.  The high byte for sprites 8--15 is exposed at
\addr{A0EB}.

\begin{warningbox}
Writing any value to this register clears the low collision byte.  BASIC
\texttt{SPRCOLL} reads and clears both bytes.
\end{warningbox}
...
\mementry{A00C}{RegColBg}{R/WC}{0}

Sprite-to-background collision bitmask.  Bit $N$ is set when sprite $N$
overlaps a non-zero pixel in the graphics bitmap.  This register covers
sprites 0--7; the high byte for sprites 8--15 is exposed at \addr{A0EC}.
...
character ultimately arrives here.  The VGC writes the current packed colour
attribute and text style flags beside the character cell.
...
\mementry{A0E6}{RegTextFlags}{R/W}{0}

Text output flags used by \addr{A00E}, \texttt{GTEXT}, and BASIC
\texttt{PRINT}. Bit~0 enables reverse text. Bit~1 makes reverse text use
\addr{A0E7} instead of the default foreground/background swap. Bit~2 marks
newly printed cells as flashing. Bit~3 marks newly printed cells and
\texttt{GTEXT} output as bold.

\begin{lstlisting}
POKE $A0E6,1 : REM reverse output
POKE $A0E6,0 : REM normal output
\end{lstlisting}

% --- $A0E7 RegTextReverseAttr ------------------------------------------------

\mementry{A0E7}{RegTextReverseAttr}{R/W}{F0}

Packed reverse text colours used when \addr{A0E6} bit~1 is set. The high
nibble is background and the low nibble is foreground.

\begin{lstlisting}
POKE $A0E7,$A3 : POKE $A0E6,3 : REM bg=10, fg=3
\end{lstlisting}

% --- $A0EB RegColStHi --------------------------------------------------------

\mementry{A0EB}{RegColStHi}{R/WC}{0}

High byte of the sprite-to-sprite collision bitmask.  Bit 0 represents sprite
8, bit 7 represents sprite 15.  Writing any value clears this byte.

% --- $A0EC RegColBgHi --------------------------------------------------------

\mementry{A0EC}{RegColBgHi}{R/WC}{0}

High byte of the sprite-to-background collision bitmask.  Bit 0 represents
sprite 8, bit 7 represents sprite 15.  Writing any value clears this byte.
...
Positions a sprite at the given Nova canvas coordinates.  Sprite X is an
unsigned 16-bit coordinate and sprite Y is an unsigned 8-bit coordinate.
The visible canvas is X=0--319 and Y=0--199; a 16 by 16 sprite is fully visible
at top-left positions X=0--304 and Y=0--184.  Values outside the visible canvas
are clipped and do not draw into the hardware border.
...
sources raised by copper pseudo-register \texttt{\$FE}.  Bit 5 is
sprite-to-sprite collision and bit 6 is sprite-to-background collision.
...
\mementry{A0F3}{RegIrqValid}{R}{\$7F}
...
\begin{itemize}
  \item \$02 --- Switch to the active primary runtime ROM.
  \item \$04 --- Switch to the extension/module ROM bank.
  \item \$05 --- Switch to NovaLogo in hosted builds that provide it.
  \item \$06 --- Switch to NovaForth in hosted builds that provide it.
\end{itemize}
...
unsigned 8-bit value.  The visible sprite coordinate space matches the
320 by 200 Nova graphics canvas.
\end{notebox}
