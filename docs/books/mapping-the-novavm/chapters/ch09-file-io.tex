...
Files are stored on mounted Nova Disk Image (NDI) devices such as
\texttt{FD0:}, \texttt{FD1:}, \texttt{HD0:}, and \texttt{HD1:}.  Bare filenames
use the current device and directory.  If no disk is mounted, file commands fail
with a not-mounted error instead of falling back to a host directory.  The FIO
automatically appends the appropriate file extension (\texttt{.bas},
\texttt{.sid}, \texttt{.gfx}, etc.) based on the command.
...
\$B9A8 & FioSizeL    & R   & 0    & Loaded data size low byte; directory size byte 0 \\
\$B9A9 & FioSizeH    & R   & 0    & Loaded data size high byte; directory size byte 1 \\
\$B9AA & FioGSpace / FioSize2 & R/W & 0 & Graphics space selector; directory size byte 2 after DirRead \\
...
\section{Boot Runtime Flags}
\label{sec:boot-runtime-flags}

The bytes at \$B9F0--\$B9FF are reserved for boot coordination between the
host and the active language ROM. They are not part of the FIO register bank,
but they live adjacent to it so all runtimes can use the same convention.
...
\textbf{Address} & \textbf{Name} & \textbf{R/W} & \textbf{Default} & \textbf{Description} \\
\midrule
\$B9F0 & AutobootSkip   & R/W & 0 & Non-zero disables startup autoboot \\
\$B9F1 & AutobootActive & R/W & 0 & Runtime is entering an autoboot program \\
\bottomrule
\end{tabular}
\end{center}

Runtimes should set \texttt{AutobootActive} before entering an autobooted
program and clear it on normal warm start. If the runtime's central error path
sees this flag set, it should clear the flag, discard the failed autoboot, and
fall back to its normal prompt without printing an error. This keeps missing or
broken startup media from looking like a machine failure.
...
\textbf{Load} command.  Directory reads use FioSizeL/FioSizeH/FioSize2 as a
24-bit little-endian file size.

\mementry{B9A9}{FioSizeH}{R}{0}

High byte of the loaded data size, or byte 1 of a directory entry's 24-bit
file size.

% --- $B9AA FioGSpace -------------------------------------------------------

\mementry{B9AA}{FioGSpace / FioSize2}{R/W}{0}
...
4 & \texttt{.nvg}/\texttt{.gfx} --- Graphics asset \\
5 & Directory \\
6 & \texttt{.4th} --- Forth source file \\
\bottomrule
\end{tabular}
\end{center}
...
buffer, the 24-bit file size in FioSizeL/FioSizeH/FioSize2
(\$B9A8--\$B9AA), and the file type in FioDirType (\$B9AF).  When all entries
have been read, FioStatus is set to \$03 with ErrCode $=$ 3 (end of
directory).

\textbf{Parameters:} None (directory must be opened with DirOpen first).

\textbf{Result:} FioName, 24-bit FioSize, and FioDirType populated, or
ErrCode $=$ 3 at end.
