...
For a static filename, \novacode{I\_FIO\_NAME} is denser: place a
zero-terminated filename immediately after the call. The routine copies it into
the same mailbox and resumes after the terminating zero.
...
When implementations are included as source, include
\novacode{fio\_inline.s} before \novacode{fio.s}; when linking
\novacode{nova.lib}, the archive resolves both members automatically.
