...
Nova provides three palette modes:

\begin{description}
\item[\novacode{VGC\_PALMODE\_C64}] The default Nova/C64-style fixed palette.
\item[\novacode{VGC\_PALMODE\_EGA}] The IBM EGA 16-color fixed palette.
\item[\novacode{VGC\_PALMODE\_CUSTOM}] A caller-supplied 16-color RGB palette.
\end{description}

Use \novacode{vgc\_palette.inc} for declarations and
\novacode{vgc\_palette.s} for the helper routines. The fixed palette helpers
return \novacode{A=0}:
...
A custom palette is exactly \novacode{VGC\_CUSTOM\_PALETTE\_BYTES} bytes: 16
RGB entries in byte order \novacode{R0,G0,B0,R1,G1,B1,...,R15,G15,B15}. Callers
may store full RGB888 values. Current FPGA video output uses the high nibble of
each channel, so the visible hardware palette is RGB444, or 4096 possible
colors. Emulators and future display paths may use more of the submitted RGB
precision.
...
\novacode{VGC\_PALIDX}, streams the bytes through \novacode{VGC\_PALDATA}, then
selects \novacode{VGC\_PALMODE\_CUSTOM}.
