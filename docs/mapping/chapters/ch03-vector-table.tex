...
% ── CharRamBase ──────────────────────────────────────────────────────────────
\mementry{0204}{CharRamBase (low)}{R/W}{\$00}

\mementry{0205}{CharRamBase (high)}{R/W}{\$AA}

Points to \$AA00, the start of the 2000-byte character RAM (80 columns
$\times$ 25 rows).  Each byte holds an ASCII character code.
See Chapter~\ref{addr:AA00} for screen memory details.

\begin{lstlisting}
CB = DEEK($0204)
POKE CB, ASC("H") : REM put 'H' at top-left
\end{lstlisting}

% ── ColorRamBase ─────────────────────────────────────────────────────────────
\mementry{0206}{ColorRamBase (low)}{R/W}{\$D0}

\mementry{0207}{ColorRamBase (high)}{R/W}{\$B1}

Points to \$B1D0, the start of the 2000-byte color RAM.  Each byte holds the
foreground colour for the corresponding character position.
See Chapter~\ref{addr:AA00}.
...
\$0204 & CharRamBase  & \$AA00 & Character RAM \\
\$0206 & ColorRamBase & \$B1D0 & Color RAM \\
