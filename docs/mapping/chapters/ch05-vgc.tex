...
an 80$\times$25 text screen, a 320$\times$200 pixel bitmap with 16 colours, and
16 multicolour 16$\times$16 hardware sprites.  All three layers are composited
at 60\,Hz into a single frame, with the rendering order determined by the
active display mode.
...
\midrule
0 & Text only     & 80$\times$25 text screen.  No graphics bitmap is drawn.
...
immediately.  Parameters must be loaded into P0--P13 (\$A011--\$A01E)
\emph{before} writing the command byte.

% --- $A011--$A01E RegP0--RegP13 ---------------------------------------------

\memrange{A011}{A01E}{RegP0--RegP13 (Parameter Registers)}{R/W}{0}

Fourteen parameter registers, addressed as \$A011 (P0) through \$A01E (P13).
...
Positions a sprite at the given screen coordinates.  Coordinates can extend
beyond the visible area for smooth entry/exit.
...
\midrule
P0       & Sprite number (0--15) \\
P1, P2   & X position (16-bit LE) \\
P3, P4   & Y position (16-bit LE) \\
\bottomrule
\end{tabular}
\end{center}
...
40 POKE $A014,100  : POKE $A015,0 : REM Y=100
...
Memory space values:
\begin{itemize}
  \item \$00 = Screen RAM (2000 bytes, character codes)
  \item \$01 = Colour RAM (2000 bytes, per-cell colour attributes)
  \item \$02 = Graphics bitmap (64000 bytes, 320$\times$200 pixels)
  \item \$03 = Sprite shape RAM (256 slots $\times$ 128 bytes = 32768 bytes)
\end{itemize}

\begin{lstlisting}
10 REM read the character at screen position 0
20 POKE $A011,0  : REM space = screen RAM
...
Copper.  It executes a list of register-write events keyed to specific
scanline positions, allowing per-scanline colour changes, split-screen
effects, and raster bars --- all without CPU intervention.

The VGC supports 128 copper lists, each holding up to 256 events.  One list
is \emph{active} (being executed each frame) and one is the \emph{target}
(being edited).  This double-buffering allows glitch-free list updates.
...
\textbf{Param} & \textbf{Meaning} \\
\midrule
P0, P1 & Scanline (16-bit LE, 0--199) \\
P2     & Column \\
P3, P4 & Target register address (16-bit LE) \\
...
20 POKE $A011,100 : POKE $A012,0  : REM scanline 100
30 POKE $A013,0                    : REM column 0
...
% ═══════════════════════════════════════════════════════════════════════════════
\section{IRQ Control Register (\$A01F)}
\label{sec:vgc-irq}
% ═══════════════════════════════════════════════════════════════════════════════

\mementry{A01F}{RegIrqCtrl}{R/W}{0}

Controls raster-triggered interrupts from the VGC.

\bitfield{---}{---}{---}{---}{---}{---}{---}{RSTR}

\begin{itemize}
  \item \textbf{Bit 0 (RSTR)}: Raster IRQ enable.  When set, the VGC fires an
        IRQ when the raster beam reaches the trigger scanline.  This allows
        frame-synchronised processing and split-screen effects.
\end{itemize}

\begin{lstlisting}
POKE $A01F,1 : REM enable raster IRQ
POKE $A01F,0 : REM disable raster IRQ
\end{lstlisting}

\begin{tipbox}
Combine the raster IRQ with the copper for advanced effects: use the IRQ
handler to swap copper lists mid-frame, creating complex multi-zone colour
schemes without burning CPU cycles in a polling loop.
\end{tipbox}
...
+2 & SprRegYLo       & R/W & Y position low byte \\
+3 & SprRegYHi       & R/W & Y position high byte \\
...
\subsection*{Direct Register Access Example}

\begin{lstlisting}
10 REM position sprite 3 at (100, 50) via direct register access
20 BASE = $A040 + 3*8
30 POKE BASE+0, 100 : POKE BASE+1, 0  : REM X=100
40 POKE BASE+2, 50  : POKE BASE+3, 0  : REM Y=50
50 POKE BASE+5, $80                    : REM enable
\end{lstlisting}

\begin{lstlisting}
10 REM read sprite 5's current position
20 BASE = $A040 + 5*8
30 X = PEEK(BASE+0) + PEEK(BASE+1)*256
40 Y = PEEK(BASE+2) + PEEK(BASE+3)*256
50 PRINT "SPRITE 5 AT ";X;",";Y
\end{lstlisting}

\begin{tipbox}
Direct register access is faster than using the command interface for
positioning sprites in a tight game loop.  Updating the X and Y registers
directly avoids the overhead of loading the parameter registers and issuing a
command.
\end{tipbox}
...
\$A011--\$A01E  & Parameter registers P0--P13                & R/W \\
\$A01F          & IRQ control                                & R/W \\
