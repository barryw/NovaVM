...
const ASSET_VERSION = new URL(self.location.href).searchParams.get("v") || "2026-06-12-novachess-tt";
...
async function initialize(canvas) {
    try {
        ...
        const [extensionBytes, libcallBytes, moduleEntries, systemDiskBytes] = await Promise.all([
            ...
            fetchOptionalBytes(SYSTEM_DISK.path)
        ]);
        ...
        if (!exports.nova_rom_ptr || !exports.nova_load_primary_rom || !exports.nova_libcall_ptr || !exports.nova_module_ptr ||
        ...
        if (systemDiskBytes) {
            mountSystemDisk(systemDiskBytes);
        }
...
async function fetchOptionalBytes(path) {
    try {
        return await fetchBytes(path);
    } catch (error) {
        postMessage({ type: "log", message: `optional asset ${path} unavailable: ${formatError(error)}` });
        return null;
    }
}

function mountSystemDisk(bytes) {
    const drive = SYSTEM_DISK.drive;
    storageDevices[drive] = new NdiDisk(bytes, SYSTEM_DISK.name);
    defaultDrive = drive;
    mountedMedia[drive] = {
        name: SYSTEM_DISK.name,
        size: bytes.byteLength || bytes.length || 0,
        modified: false,
        system: true
    };
...
function fioLoadRuntime() {
    const filename = fioFilename();
    try {
        const data = loadDataFile(filename, ".bin");
        loadPrimaryRuntime(data);
        setFioSize24(data.length);
        return;
    } catch (error) {
        if (!(error instanceof FioError) || error.code !== FIO_ERR_NOT_FOUND) throw error;
    ...
    loadPrimaryRuntime(runtimeAssets[runtime[0]]);
    setFioSize24(runtimeAssets[runtime[0]].length);
}

function loadPrimaryRuntime(data) {
    if (data.length !== ROM_SIZE) {
        throw new FioError(FIO_ERR_IO);
    }
    new Uint8Array(memory.buffer, exports.nova_rom_ptr(), data.length).set(data);
    exports.nova_load_primary_rom(data.length);
    ram.set(data, 0xC000);
}
